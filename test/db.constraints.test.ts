...
describe('Database Constraints', () => {
  ...
  describe('Unique constraints', () => {
    it('should enforce unique job gitLabJobId', async () => {
      ...
      await expect(db.insert(schema.jobs).values(newJob)).rejects.toThrow();
    });
  ...
  describe('Cascade delete', () => {
    ...
    it('should cascade delete from procedures to maxAllowedDays and waitingPeriods', async () => {
      // First verify the maxAllowedDays exists
      const beforeDeleteMAD = await db
        .select()
        .from(schema.maxAllowedDays)
      ...
      await db
        .delete(schema.procedures)
        .where(eq(schema.procedures.id, fixtures.procedure.id));

      // Check that the maxAllowedDays was also deleted
      const afterDeleteMAD = await db
        .select()
        .from(schema.maxAllowedDays)
        .where(eq(schema.maxAllowedDays.procedureId, fixtures.procedure.id));
