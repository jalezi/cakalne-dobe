...
describe('Database Schema', () => {
    ...
    it('should insert and retrieve jobs correctly', async () => {
        ...
        const jobs = await db.select().from(schema.jobs).where(eq(schema.jobs.id, fixtures.job.id));
    ...
    it('should insert and retrieve institutions correctly', async () => {
        ...
        const institutions = await db.select().from(schema.institutions).where(eq(schema.institutions.id, fixtures.institution.id));
    ...
    it('should insert and retrieve procedures correctly', async () => {
        ...
        const procedures = await db.select().from(schema.procedures).where(eq(schema.procedures.id, fixtures.procedure.id));
    ...
    it('should insert and retrieve maxAllowedDays correctly', async () => {
        ...
        const maxAllowedDays = await db.select().from(schema.maxAllowedDays).where(
            eq(schema.maxAllowedDays.jobId, fixtures.maxAllowedDays.jobId)
        );
        ...
        expect(maxAllowedDays[0].procedureId).toBe(fixtures.maxAllowedDays.procedureId);
    ...
    it('should insert and retrieve waitingPeriods correctly', async () => {
        ...
        const waitingPeriods = await db.select().from(schema.waitingPeriods).where(
            eq(schema.waitingPeriods.jobId, fixtures.waitingPeriod.jobId)
        );
        ...
        expect(waitingPeriods[0].institutionId).toBe(fixtures.waitingPeriod.institutionId);
        expect(waitingPeriods[0].procedureId).toBe(fixtures.waitingPeriod.procedureId);
    ...
    it('should maintain relationships between tables', async () => {
        ...
        const jobWaitingPeriods = await db.select().from(schema.waitingPeriods).where(
        ...
        const institutionWaitingPeriods = await db.select().from(schema.waitingPeriods).where(
        ...
        const procedureWaitingPeriods = await db.select().from(schema.waitingPeriods).where(
        ...
        const jobMaxAllowedDays = await db.select().from(schema.maxAllowedDays).where(
        ...
        const procedureMaxAllowedDays = await db.select().from(schema.maxAllowedDays).where(
